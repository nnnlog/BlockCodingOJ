...
module.exports = async (data) => {
	try {
		...
		global.updateProblemData.push(async () => {
			let user = await db.user().findOne({
				user_id: data.user_id,
			});
			if (user === null) {
				assert(`User ${data.user_id} not found.`);
				return;
			}
			if (user.accepted[data.problem_id] === undefined) {
				user.accepted[data.problem_id] = 1;
				...
				let prob = await db.problem().findOne({
					problem_id: data.problem_id,
				});
				if (prob === null) {
					assert(`Problem ${data.problem_id} not found.`);
					return;
				}
				prob.accepted[data.user_id] = 1;
